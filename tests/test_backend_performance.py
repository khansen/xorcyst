#!/usr/bin/env python3
"""Check AST work bounds and output contracts without timing thresholds."""
import os
...
class BackendPerformance(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        objects = []
        for section in ('TEST_AST_WORK', 'TEST_BACKEND_MAIN'):
            obj = root / (section + '.o')
        ...
        sources = [REPO / name for name in sources.replace('\\\n', ' ').split()
                   if name.endswith('.c') and name not in ('astnode.c', 'xasm.c')]
    ...
    def run_measured(self, args):
        ...
        match = re.search(rb'BACKEND_WORK index_nodes=(\d+)', result.stderr)
        self.assertIsNotNone(match, result.stderr.decode())
        return result, int(match[1])
    ...
    def test_removal_preserves_tree_links_and_subtrees_with_linear_work(self):
        for count in (512, 2048):
            with self.subTest(nodes=count):
                _, nodes = self.run_measured(['--test-ast-removal', count])
            ...
            for count in (512, 2048):
                ...
                for complete in (False, True):
                    ...
                    with self.subTest(statements=count, complete_xref=complete):
                        _, nodes = self.run_measured(['--pure-binary', source, '-o', output, *flags])
